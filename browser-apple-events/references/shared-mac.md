...
- **Focus or window order changes:** keep going by ID. Do not activate the browser or restore the old arrangement.
...
- **Page is hidden or minimized:** ordinary DOM work may still work. Do not use `document.hidden` as a reason to steal focus. If the site needs trusted input or foreground activity, ask for a manual step.
