...
For a missing dictionary: report that Apple Events support could not be established for the installed app. Do not infer support or absence from the engine or brand alone. No automatic fallback to Accessibility, CDP, WebDriver, or Playwright. If the user separately chooses a different transport, treat it as a new task outside this skill's Apple Events-only contract.
