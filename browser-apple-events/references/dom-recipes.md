...
A visible value is not necessarily committed application state. For controlled dropdowns, inspect loading, whether the menu is open, and the selected value separately. Zero `[role=option]` matches on one read does not establish an empty allowed-value list. Identity pickers can need a selection/commit step before Save; one historical ArrowDown/Enter workaround is not a universal rule. Do not add raw keys to this recipe. Compare saved fields and revisions through a fresh authorized read.
...
- For a same-origin frame, select a unique iframe and verify its document URL and time origin **inside the action call**, in addition to the top-level guard. Selectors on `document` do not enter frames. Cross-origin frame DOM access is unavailable through ordinary page JavaScript. Use the [scoped AX inspector](accessibility.md) for metadata if the bound tab is already foreground. AX writes are disabled; request a manual click or manual text entry, then verify the saved result. Do not seek another transport approval or invent a write workaround.
...
- Browser UI and native file pickers are not DOM controls. Explain the precise limitation and request a manual step. The AX inspector requires a bound web document and does not automate native pickers. Login, passkeys, MFA, and security permission grants stay with the user. No hidden fallback and no CDP.
