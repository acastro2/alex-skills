...
Be specific. Quote from transcripts where relevant.
...
Write a JSON file with comparison_summary, winner_strengths, loser_weaknesses, instruction_following scores, improvement_suggestions (with priority, category, suggestion, expected_impact), and transcript_insights.
...
3. For each expectation across all runs, analyze patterns:
   - Does it always pass in both configurations?
   - Does it always fail in both configurations?
   - Does it always pass with skill but fail without?
   - Does it always fail with skill but pass without?
   - Is it highly variable?
4. Look for patterns across evals and analyze metrics (time, tokens, tool_calls)
