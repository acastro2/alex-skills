...
# Count of distinct values of a label (e.g. how many distinct jobs exist)
count(count(up) by (job))
```
...
# Avoid division by zero: guard the denominator with `or vector(...)`
# clamp_min keeps the denominator >= 1 so the ratio stays finite
