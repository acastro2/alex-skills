...
Paths in this file, including script commands, are relative to the `alex-voice/` skill directory, even after loading a register reference. Resolve them to absolute paths before use. The existing blog and fingerprint references use reference-local `../` paths; resolve those from their own directory to the same skill files.
...
Load the linked rules and synthetic samples for the selected register before drafting or reviewing:

| Register | Load when | Reference |
| --- | --- | --- |
| Chat | Teams and Slack DMs, working threads | [Chat rules and samples](references/registers/chat.md) |
| Comms | Email, Teams channel posts, announcements, asks with deadlines | [Comms rules and samples](references/registers/comms.md) |
