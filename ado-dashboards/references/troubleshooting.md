...
- **Double-encoded `settings` string — accepted silently, breaks rendering.** Building `settings` in a shell/jq pipeline as `jq -n '{...} | tostring'` (no `-r`) captures a value that ALREADY has outer quotes/escaping baked in from jq's own JSON-string serialization, then passing that via `--arg settings "$var"` into the widget body double-wraps it: the API stores and echoes it back with no error, but `.settings | fromjson` yields another **string**, not the settings object — confirmed on QueryScalarWidget, WitChartWidget, and WitViewWidget in the same session, 2026-07. Detection: after any widget create/update, `GET` it back and check `.settings | fromjson | type` — it must be `"object"` (or, for Markdown, `.settings` itself must be the raw string with no JSON quoting at all). If it comes back `"string"`, the widget will pass every API check and still fail to render in the browser (same failure class as the missing-`selectedColumns` trap above, different root cause). Fix: build the settings object with `jq -nc '{...}'` (compact, no `tostring`) and use that raw JSON text as the `--arg` value for the outer widget body — that gives exactly one layer of encoding.
- **Markdown widget content via `--arg` swallows real newlines.** `jq --arg` does not interpret `\n` escapes typed into a shell string — passing a two-line message this way produces a literal backslash-n in the rendered widget instead of a line break. Build the markdown value as a real jq string literal (e.g. `jq -n '"line one\nline two"'`, where jq itself parses the escape) or with `printf`/a heredoc containing an actual newline, not `--arg` fed a shell string containing `\n` text.
