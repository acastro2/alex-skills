...
| Query Tile (count/scalar) | `ms.vss-dashboards-web.Microsoft.VisualStudioOnline.Dashboards.QueryScalarWidget` | `{"queryId":"<guid>","queryName":"<name>"}` when uncoloured. **Coloured** (harvested 2026-09 from Tiger / "Overview" / "# Total SE Open Bugs", human-built, renders): `{"queryId":"<guid>","queryName":"<name>","defaultBackgroundColor":"#009ccc","colorRules":[],"lastArtifactName":"<string>"}` — `defaultBackgroundColor` is the tile colour and is **safe to PATCH** (5 tiles set and read back on the Change Control dashboard, 2026-09). `lastArtifactName` does **not** mirror `queryName` in the one coloured instance, so don't assume it names the query. | **flat**, no `"query"` wrapper — different shape from WitViewWidget despite both referencing a query. `colorRules` is present but **empty in every instance org-wide** (11 Query Tiles swept across the 8 projects the token can see, 2026-09), so the rule element shape is still unconfirmed — see Gotcha below. **Size is locked to 1×1** — a live create attempt at 2×1 was rejected server-side with `VS402508: size (2x1) not supported for widgets of that type`. Don't request a non-1x1 size for this widget type. |
...
**Half confirmed, 2026-09.** A Query Tile's `settings` carries `colorRules` alongside `defaultBackgroundColor`. `defaultBackgroundColor` is proven: `#009ccc` on a live Tiger tile, and 5 tiles were re-coloured by PATCH on the Change Control dashboard and read back correctly — so a static per-tile colour is supported through the API.

The **rule element shape inside `colorRules` is still unconfirmed**: it was `[]` in all 11 Query Tiles found org-wide (8 projects visible to the token), including every tile on active dashboards. So do not guess the element keys. If you need thresholds, configure one manually via the ADO web UI on a test tile, then GET its widget object and diff the `settings` string — that diff is the answer.
