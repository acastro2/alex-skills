...
**In an Attain repo, the ADR shape belongs to the `ea-doc-framework` skill, not to this one.** Read that skill and use its template. `ADR-FORMAT.md` here is the lightweight shape for personal repos, and its rules (one decision, immutable once Accepted, no invented fields) still apply.
